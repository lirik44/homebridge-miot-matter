...
const LegacyLocalProtocol = require('./LegacyLocalProtocol.js');
...
class MiotDevice extends EventEmitter {
  ...
  setLegacyLocalProtocol(spec) {
    // Some devices are described by a MIoT spec and reachable through the cloud with it, but
    // locally only understand the commands they shipped with. This is how to talk to those.
    this.legacyLocalProtocol = spec
      ? new LegacyLocalProtocol(spec, (method, params) => this.miioProtocol.send(this.ip, method, params), this.logger)
      : null;
  }

  usesLegacyLocalProtocol() {
    return !!this.legacyLocalProtocol && this.isConnectedToLocalDevice() && !(this.shouldUseMiCloud() && this.isConnectedToCloud());
  }
  ...
  async getMiotProperties(params = []) {
    ...
    } else if (this.usesLegacyLocalProtocol()) {
      return this.legacyLocalProtocol.readProperties(params);
  ...
  async setMiotProperties(params = []) {
    if (this.shouldUseMiCloud() && this.isConnectedToCloud()) {
      ...
      return this.legacyLocalProtocol.writeProperties(params);
    } else if (this.isConnectedToLocalDevice()) {
  ...
  async miotAction(param = {}) {
    if (this.shouldUseMiCloud() && this.isConnectedToCloud()) {
      ...
      return this.legacyLocalProtocol.runAction(param);
    } else if (this.isConnectedToLocalDevice()) {
