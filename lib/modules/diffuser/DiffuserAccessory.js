...
class DiffuserAccessory extends BaseAccessory {
  ...
  setupAdditionalAccessoryServices() {
    ...
    if (this.getDevice().supportsFragranceDeliveryAction()) {
      this.fragranceDeliveryService = this.createStatlessSwitch(this.getName() + ` Fragrance Delivery`, 'fragranceDeliveryService', this.executeFragranceDelivery);
      this.addAccessoryService(this.fragranceDeliveryService);
    }
  ...
  executeFragranceDelivery(state) {
    if (this.isMiotDeviceConnected()) {
      this.getDevice().fireFragranceDelivery();
    } else {
      throw new HapStatusError(HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  }
