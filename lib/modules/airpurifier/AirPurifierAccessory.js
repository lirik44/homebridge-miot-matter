...
class AirPurifierAccessory extends BaseAccessory {
  ...
  getRotationSpeed() {
    ...
    if (this.getDevice().isFavoriteModeEnabled()) {
      return this.getDevice().getFavoriteSpeedPercentage();
    }

    // In any other mode the slider shows what the purifier is actually doing rather than zero.
    // HomeKit reads a zero as "switched on but not running" and helpfully sends 100% the next
    // time the tile is tapped - which lands here, drops the purifier out of auto into favourite
    // and sets it roaring at full speed, when all anyone did was switch it on.
    return this.getDevice().getRotationSpeedPercentage();
  }

  setRotationSpeed(value) {
    if (this.isMiotDeviceConnected()) {
      // Asking for the speed already shown is HomeKit restoring its slider, not a person moving
      // it; a person moving it to where it already is has asked for nothing either way.
      if (Math.round(Number(value)) === Math.round(Number(this.getRotationSpeed()))) {
        return;
      }
