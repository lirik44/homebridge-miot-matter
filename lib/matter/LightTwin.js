...
class LightTwin extends MatterTwin {
  ...
  handlers() {
    ...
    if (this._hasBrightness()) {
      const setLevel = async (request) => {
        ...
        if (this.isOwnUpdate('levelControl', 'currentLevel', level)) {
          return;
        }
        await this.device.setBrightness(this.fromMatterLevel(level));
      };
    ...
    if (this._hasColorTemperature()) {
      handlers.colorControl = {
        moveToColorTemperatureLogic: async (request) => {
          ...
          if (this.isOwnUpdate('colorControl', 'colorTemperatureMireds', mireds)) {
            return;
          }
  ...
  async _turn(on) {
    if (this.isOwnUpdate('onOff', 'onOff', on)) {
      return;
    }
