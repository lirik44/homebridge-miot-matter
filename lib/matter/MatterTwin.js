...
/**
 * How long after reporting a cluster anything arriving on it is taken for the controller keeping
 * its own attributes in step rather than a person asking for something.
 */
...
class MatterTwin {
  ...
  /**
   * @returns {object} What the accessory is registered with. A report may carry only what
   *   changed, but registration has to carry everything the clusters need to exist at all.
   */
  initialState() {
    return this.state();
  }
  ...
  report() {
    ...
    Object.keys(state).forEach((cluster) => {
      ...
      Promise.resolve(this.api.matter.updateAccessoryState(this.uuid, cluster, changed))
        .catch(err => this.logger.warn(`Could not report ${cluster} over Matter: ${err.message || err}`));
      // A change is an event, not a poll: worth a line in the log of a plugin that is otherwise
      // quiet about them.
      this.logger.info(`Matter: ${this.name} ${cluster} = ${JSON.stringify(changed)}`);
    });
  }
   ...
   */
  /**
   * Whether this plugin has just reported this cluster.
   *
   * Reporting one attribute makes a controller derive others - report a fan's speed and it works
   * out the mode, report the mode and it works out a speed of its own choosing - and each of those
   * arrives here looking exactly like a command. Anything landing this soon after a report is the
   * controller tidying up after us, not a person: a person's command comes out of nowhere.
   *
   * @param {string} cluster The cluster a change arrived on.
   * @returns {boolean} Whether to leave it alone.
   */
  justReported(cluster) {
    const at = this.pushedAt[cluster] || 0;
    const settling = Date.now() - at < SETTLE_MS;

    if (settling) {
      this.logger.debug(`Matter: ignoring a change to ${cluster}, it followed this plugin's own report`);
    }
