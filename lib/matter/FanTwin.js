    ...
    return {
      fanControl: {
        fanModeChange: async (request) => {
          ...
          this.logger.debug(`Matter: ${this.name} asked for fan mode ${asked}${this.justReported('fanControl') ? ' (ignored, it followed our own report)' : ''}`);
        ...
        percentSettingChange: async (request) => {
          ...
          this.logger.debug(`Matter: ${this.name} asked for ${asked}%${this.justReported('fanControl') ? ' (ignored, it followed our own report)' : ''}, device is ${this.device.isOn() === true ? `on at ${this._speed()}%` : 'off'}`);
          if (this.justReported('fanControl')) {
            return;
          }
