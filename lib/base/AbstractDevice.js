...
class AbstractDevice extends EventEmitter {
  ...
  async initDevice(propertyChunkSize) {
    ...
    // ... and how to talk to it locally, for a device that only understands the old commands
    this.getMiotDevice().setLegacyLocalProtocol(this.legacyLocalProtocol());
  ...
  /**
   * What this device understands on the local network, for one that answers the old miIO commands
   * rather than the MIoT ones - a Yeelight lamp, say, which takes `set_power` but not a MIoT
   * property write. Returning nothing means it speaks MIoT locally, as most devices do.
   *
   * @returns {object|null} `properties` keyed by "siid.piid" and optionally `actions` keyed by
   *   "siid.aiid"; see LegacyLocalProtocol.
   */
  legacyLocalProtocol() {
    return null;
  }
  ...
  async setPropertyValue(propObj, value) {
    ...
    if (prop) {
      ...
      if (prop.getValue() !== adjustedValue) {
        this.logger.debug(`Writing ${prop.getName()} = ${JSON.stringify(adjustedValue)} (was ${JSON.stringify(prop.getValue())})`);
