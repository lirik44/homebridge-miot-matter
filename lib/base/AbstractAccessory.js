...
class AbstractAccessory {
  ...
  addLightService(serviceId, serviceName, miotService) {
    ...
    if (newLightService) {
      const lightService = this.initAndAddCustomService(newLightService);
      this.setupAdaptiveLighting(lightService);
      return lightService;
    }
    return null;
  }
  ...
  /**
   * Lets HomeKit move the colour temperature through the day on its own - warm in the evening,
   * cold at noon - which it calls adaptive lighting. HomeKit does the following; all the lamp has
   * to do is accept a brightness and a colour temperature, which is exactly what makes it eligible.
   */
  setupAdaptiveLighting(lightService) {
    if (!lightService || !lightService.supportsAdaptiveLighting || !lightService.supportsAdaptiveLighting()) {
      return;
    }

    if (this.getConfigValue('adaptiveLightingControl', true) === false) {
      this.logger.debug(`Adaptive lighting is turned off in the config for ${lightService.getServiceName()}`);
      return;
    }

    const AdaptiveLightingController = this.api.hap.AdaptiveLightingController;
    const accessory = this.getAccessory();
    if (!AdaptiveLightingController || !accessory || typeof accessory.configureController !== 'function') {
      this.logger.debug(`This Homebridge cannot do adaptive lighting`);
      return;
    }

    try {
      this.adaptiveLightingController = new AdaptiveLightingController(lightService.getLightbulbService());
      accessory.configureController(this.adaptiveLightingController);
      this.logger.info(`Adaptive lighting enabled for ${lightService.getServiceName()}`);
    } catch (err) {
      this.logger.warn(`Could not enable adaptive lighting: ${err.message || err}`);
    }
  }
